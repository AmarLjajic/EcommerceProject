    ...
    public class ProductController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ProductModel>>> GetProducts()
        {
            ...
            foreach (var item in await _context.Products.Include(x => x.Category).ToListAsync())
                items.Add(new ProductModel(item.Id, item.Name, item.Description, item.Price, item.Category.Name));

            return items;
        }

        [HttpGet("{id}")]
        public async Task<ActionResult<CategoryEntity>> GetDetaila(int id)
        {
            ...
            return new CategoryEntity(item.Id, item.Name, item.Products);
        }
    }    
}
